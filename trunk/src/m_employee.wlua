...
function Employee:__init()
	...
	self.btn_edit = iup.button{ title = "编辑",action=function() self.employee_edit = EmployeeEdit(self,false)end}
	self.btn_add = iup.button{ title = "添加",action=function() self.employee_edit = EmployeeEdit(self,true)end}
	...
	self.dlg = iup.dialog{
	  iup.vbox{
		...
		title = "添加员工", resize = "YES", menubox = "NO", maxbox = "NO", minbox = "NO" }
	self:refresh()
...
function EmployeeEdit:__init(employee,isadd)
	...
	self.label =iup.label{title="员工添加",size=100}
